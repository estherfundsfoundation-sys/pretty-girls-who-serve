(() => {
  ...
  const message = (id, value, bad = false) => {
    ...
    if (element) {
      ...
      element.style.color = bad ? "#a52c4e" : "";
    }
  };
  const escape = (value = "") =>
  ...
  async function api(path, options = {}) {
    ...
    if (!response.ok) {
      const error = new Error(
        body.error || "The P31 Portal could not complete that request.",
      );
  ...
  function setAuthIntent(intent) {
    ...
    $("authPassword").autocomplete = joining
      ? "new-password"
      : "current-password";
    ...
    text(
      "emailSignInLink",
      joining
        ? "Create my account by secure email link"
  ...
  function renderGate(data) {
    ...
    $("gateBenefits").innerHTML = benefits
      .map((benefit) => `<span>♡ ${escape(benefit)}</span>`)
      .join("");
    show("legacyClaim", Boolean(data.legacy?.claimAvailable));
    show("legacyReview", Boolean(data.legacy?.needsReview));
    $("startCheckout").disabled =
      !data.checkoutReady || Boolean(data.legacy?.claimAvailable);
    const checkoutState = new URLSearchParams(location.search).get("checkout");
    if (data.legacy?.claimAvailable) {
      message(
        "gateMessage",
        "We found your previous paid membership and are connecting it now. Please stay on this page.",
      );
      ...
      message(
        "gateMessage",
        "Checkout was cancelled. You were not charged and may continue whenever you are ready.",
      );
    } else if (checkoutState === "success") {
      message(
        "gateMessage",
        "Payment received. We are opening your P31 Portal now.",
      );
    } else {
      message(
        "gateMessage",
        "One secure $20 payment unlocks your lifetime P31 membership. Checkout opens separately while this page confirms your access automatically.",
      );
    }
  }
  ...
  function cards(items, empty, renderer) {
    return items?.length
      ...
      : `<article><h2>${escape(empty)}</h2><p>Updates will appear here when available.</p></article>`;
  }
  ...
  function renderPortal(data) {
    ...
    text(
      "chapterStatus",
      data.chapter?.pgws_chapters?.name || "National sister · no chapter",
    );
    text(
      "serviceHours",
      Number(data.service?.approvedHours || 0)
        .toFixed(1)
        .replace(".0", ""),
    );
    $("activateMyEff").href =
    ...
    $("nextSteps").innerHTML = [
      ...
      data.myEff?.status === "linked"
        ? null
        : "<li>Activate or connect your included MyEFF access.</li>",
      data.chapter
        ? null
        : "<li>Explore a chapter or remain connected nationally.</li>",
    ...
    $("homeEvents").innerHTML = cards(
      ...
      (event) =>
        `<article><span>${escape(dateTime(event.starts_at))}</span><h2>${escape(event.title)}</h2><p>${escape(event.description)}</p>${event.access_url ? `<a href="${escape(event.access_url)}">Open event →</a>` : ""}</article>`,
    );
    $("homeAnnouncements").innerHTML = data.announcements?.length
      ? data.announcements
          .map(
            (item) =>
              `<article><h3>${escape(item.title)}</h3><p>${escape(item.body)}</p>${item.href ? `<a href="${escape(item.href)}">Read more →</a>` : ""}</article>`,
          )
          .join("")
      : "<article><h3>No new announcements</h3><p>National and chapter updates will appear here.</p></article>";
  ...
  function renderBecoming(progress) {
    ...
    const steps = [
      [
        "welcome",
        "Welcome Home",
        "Mission, membership, and your PGWS member promise",
      ],
      [
        "faith",
        "Faith + Identity",
        "Proverbs 31 without perfection or performance",
      ],
      [
        "sisterhood",
        "Sisterhood + Safety",
        "Consent, care, conflict, privacy, and belonging",
      ],
      [
        "service",
        "Purpose in Action",
        "Ethical service, documentation, and real impact",
      ],
      [
        "chapter",
        "Chapter Connection",
        "National membership, campus chapters, and leadership",
      ],
      [
        "myeff",
        "MyEFF Connection",
        "Your included EFF national membership and product boundaries",
      ],
    ];
    $("becomingTrack").innerHTML = steps
      .map(
        ([key, title, detail], index) =>
          `<article><b>${completed.has(key) ? "✓" : String(index + 1).padStart(2, "0")}</b><div><strong>${escape(title)}</strong><small>${escape(detail)}</small></div></article>`,
      )
      .join("");
  }
  ...
  function renderService(entries) {
    $("serviceList").innerHTML = entries.length
      ? entries
          .map(
            (entry) =>
              `<article><div><h3>${escape(entry.organization_name)}</h3><p>${escape(date(entry.service_date))} · ${escape(entry.description)}</p></div><span>${Number(entry.hours)} hours · ${escape(pretty(entry.status))}</span></article>`,
          )
          .join("")
      : "<article><div><h3>No service entries yet</h3><p>Submit your first service experience above.</p></div></article>";
  }
  ...
  function renderResources(resources, selected = "All") {
    ...
    $("resourceFilter").innerHTML = categories
      .map(
        (category) =>
          `<button class="${category === selected ? "active" : ""}" data-resource-category="${escape(category)}">${escape(category)}</button>`,
      )
      .join("");
    const visible =
      selected === "All"
        ? resources
        : resources.filter((item) => item.category === selected);
    $("resourceList").innerHTML = cards(
      visible,
      "No resources in this category",
      (item) =>
        `<article><span>♡</span><h2>${escape(item.title)}</h2><p>${escape(item.description)}</p><a href="${escape(item.href)}">Open resource →</a></article>`,
    );
    document
      .querySelectorAll("[data-resource-category]")
      .forEach((button) =>
        button.addEventListener("click", () =>
          renderResources(resources, button.dataset.resourceCategory),
        ),
      );
  }

  function renderOpportunities(items) {
    $("opportunityList").innerHTML = cards(
      items,
      "No active opportunities yet",
      (item) =>
        `<article><span>${escape(pretty(item.opportunity_type))}</span><h2>${escape(item.title)}</h2><p>${escape(item.description)}</p>${item.closes_at ? `<small>Closes ${escape(date(item.closes_at))}</small>` : ""}${item.href ? `<a href="${escape(item.href)}">View opportunity →</a>` : ""}</article>`,
    );
  }

  function renderSupport(items) {
    $("supportList").innerHTML = items.length
      ? items
          .map(
            (item) =>
              `<article><div><h3>${escape(item.subject)}</h3><p>${escape(pretty(item.category))} · Opened ${escape(date(item.created_at))}</p>${item.resolution_notes ? `<p><b>PGWS response:</b> ${escape(item.resolution_notes)}</p>` : ""}</div><span>${escape(pretty(item.status))}</span></article>`,
          )
          .join("")
      : "<article><div><h3>No support requests yet</h3><p>Use the secure form when you need PGWS help.</p></div></article>";
  }
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          message(
            "gateMessage",
            `${error.message} Choose “Reconnect my existing membership” to try again.`,
            true,
          );
          return;
        } finally {
      ...
      if (data.portalAccess) {
        ...
        history.replaceState(
          {},
          "",
          panel && validPanels.has(panel)
            ? `/p31?panel=${encodeURIComponent(panel)}`
            : "/p31",
        );
        renderPortal(data);
      } else if (poll) {
        ...
        message(
          "gateMessage",
          "Stripe is still confirming your payment. Choose “Check my payment again” in a moment; you will never be charged twice from this screen.",
          true,
        );
      ...
      if (error.status === 401) {
        setView("auth");
        setAuthIntent(
          new URLSearchParams(location.search).get("intent") === "join"
            ? "join"
            : null,
        );
      } else {
  ...
  $("authForm").addEventListener("submit", async (event) => {
    ...
    if (authIntent === "join") {
      ...
      if (!displayName)
        return message(
          "authMessage",
          "Tell us the first name you would like PGWS to use when welcoming you.",
          true,
        );
      ...
      if (data.session) {
        ...
        message(
          "authMessage",
          "Your account is ready. Check your email once to verify it, then the link will bring you directly back to membership checkout.",
        );
      }
      return;
    }
    ...
    if (error)
      return message(
        "authMessage",
        `${error.message} You may also use the secure email-link option below.`,
        true,
      );
    session = data.session;
    await loadPortal();
  ...
  $("emailSignInLink").addEventListener("click", async () => {
    ...
    const displayName = $("authName").value.trim();
    if (joining && !displayName)
      return message(
        "authMessage",
        "Enter the first name you would like PGWS to use.",
        true,
      );
    ...
    message(
      "authMessage",
      error?.message ||
        "Check your email and open the secure link. It will return you directly to P31.",
      Boolean(error),
    );
  });
  $("forgotPassword").addEventListener("click", async () => {
    ...
    const { error } = await client.auth.resetPasswordForEmail(email, {
      redirectTo: `${location.origin}/p31`,
    });
    message(
      "authMessage",
      error?.message || "Check your email for the secure password-reset link.",
      Boolean(error),
    );
  });
  ...
  async function waitForPayment(checkoutWindow) {
    ...
    message(
      "gateMessage",
      "Your checkout tab may still be open. After Stripe confirms payment, choose “Check my payment again.” You will not be charged twice.",
      true,
    );
  }
  ...
  $("startCheckout").addEventListener("click", async () => {
    ...
    try {
      ...
      message(
        "gateMessage",
        "Complete checkout in the secure Stripe tab. Keep this P31 page open—it will unlock automatically, with no second login.",
      );
  ...
  $("serviceForm").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      message(
        "serviceMessage",
        "Your service entry was submitted for PGWS review.",
      );
      await loadPortal();
      switchPanel("service");
    } catch (error) {
      message("serviceMessage", error.message, true);
    }
  });
  $("supportForm").addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      await api("/api/pgws/support", {
        method: "POST",
        body: JSON.stringify({
      ...
      message(
        "supportFormMessage",
        "Your secure request was sent to PGWS Nationals.",
      );
      await loadPortal();
      switchPanel("support");
    } catch (error) {
      message("supportFormMessage", error.message, true);
    }
  });
    ...
    document
      ...
      .forEach((element) =>
        element.classList.toggle(
          "active",
          element.dataset.panelContent === panel,
        ),
      );
    document
  ...
  document
    .querySelectorAll("[data-go-panel]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        switchPanel(button.dataset.goPanel),
      ),
    );
  ...
  async function boot() {
    if (!client) {
      ...
      return message(
        "authMessage",
        "PGWS account services are unavailable.",
        true,
      );
    }
    const { data } = await client.auth.getSession();
    ...
    if (!session) {
      setView("auth");
      const requestedIntent = new URLSearchParams(location.search).get(
        "intent",
      );
      setAuthIntent(
        requestedIntent === "join" || requestedIntent === "signin"
          ? requestedIntent
          : null,
      );
      return;
